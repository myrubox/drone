...
class Simulation:
    ...
    def check_and_handle_feasibility(self):
        ...
        """
        # 1. Если маршрут еще не построен (например, при запуске миссии) ИЛИ мы вне маршрута (например, после посадки/зарядки)
        ...
        if required_energy > available_energy:
            ...
            for i, st in enumerate(self.stations):
                ...
                if energy <= available_energy and energy < min_energy:
                    ...
                    best_station = i
            if best_station is not None:
                # Строим новый маршрут: ТЕКУЩАЯ_ПОЗИЦИЯ -> станция -> конечная цель (через plan_full_mission_with_charging)
                planned_route = self.plan_full_mission_with_charging(self.drone_pos, self.end_pos)
                if planned_route is None:
                    self.update_log(
