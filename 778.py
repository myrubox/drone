...
class Simulation:
    ...
    def total_route_energy(self, route_points, heights=None):
        ...
        total = 0.0
        if heights is None:
            heights = [self.drone_height] * len(route_points)
    ...
    def check_and_handle_feasibility(self):
        ...
        # ---- ОТЛАДКА ----
        self.update_log(f"Маршрут: {self.route_points}")
        self.update_log(f"Текущий индекс: {self.current_route_index}")
        self.update_log(f"Текущая позиция: {self.drone_pos}, цель: {self.target_pos}")
        if len(self.route_points) >= 3:
            e1 = self.calculate_energy_consumption(self.route_points[0], self.route_points[1], self.drone_height,
                                                   self.drone_height)
            e2 = self.calculate_energy_consumption(self.route_points[1], self.route_points[2], self.drone_height,
                                                   self.drone_height)
            self.update_log(f"Проверка вручную: {e1:.2f} + {e2:.2f} = {e1 + e2:.2f}")
        # ---- КОНЕЦ ОТЛАДКИ ----
