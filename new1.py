...
class DroneMotionController:
    """
    Класс для управления движением дрона с реалистичным разгоном и торможением.
    Поддерживает плавный старт, выход на крейсерскую скорость и замедление к целевой точке.
    """
    def __init__(
        self,
        mass=0.9,
        ...
        accel_horizontal=4.0,    # м/с² (ускорение)
        decel_horizontal=4.5,    # м/с² (торможение)
        accel_vertical=1.5,      # м/с²
        decel_vertical=1.8       # м/с²
    ):
        self.mass = mass
        ...
        self.axis = 'horizontal'  # 'horizontal' или 'vertical'
    ...
    def update(self, dt):
        """
        Выполнить один шаг интеграции движения на dt секунд.
        """
        ...
        # Нормированный вектор направления
        dir_norm = direction / distance

        # Текущая скорость вдоль направления
        ...
        # Расстояние для торможения до нуля с текущей скорости
        braking_dist = (v_along ** 2) / (2 * a_decel)
        ...
        # Если очень близко к цели и скорость маленькая — "прилипнуть" к цели
        if distance < 0.01 and v_target < 0.05:
            self.pos = self.target.copy()
            self.vel *= 0

    def get_speed(self):
        """Текущая абсолютная скорость (м/с)."""
...
class Simulation:
    ...
    def reset_simulation(self, event=None):
        ...
        self.update_log("Симуляция сброшена.", level="info")
    ...
    def handle_arrival(self) -> None:
        ...
        if hasattr(self, "route_points") and hasattr(self, "current_route_index"):
            if self.current_route_index + 1 < len(self.route_points):
                ...
                # ЗАЩИТА ОТ ВЫХОДА ЗА ГРАНИЦЫ МАРШРУТА
    ...
    def move_drone(self, *args) -> list:
        """
        Реалистичное движение дрона с учетом физики, скорости в метрах, размеров карты и плавного разгона/торможения.
        Дрон перемещается по карте, которая разделена на клетки (cell_size метров).
        """
        if self.is_charging or not self.mission_active or self.target_pos is None:
            return [self.drone_icon, self.route_line]

        # Проверка на невалидную цель (например, (0,0), если это не стартовая точка)
        ...
        # --- Преобразование координат из клеток в метры ---
        # ВАЖНО: drone_pos, target_pos, _start_pos_of_leg - в клетках! Преобразуем в метры для физики.
        ...
        # Если стартует новый этап движения (новая цель)
        if not hasattr(self, "_last_energy_check_target") or not np.allclose(self._last_energy_check_target,
            ...
            # MotionController: новая задача, позиции и цели в метрах!
            self.motion_controller.set_position(current_pos_m)
            self.motion_controller.set_target(target_pos_m)
            # Оставляем текущую скорость (для разгона/торможения), но при сбросе/старте можно обнулить:
            # self.motion_controller.set_velocity([0, 0])

        # Проверка энергии на весь этап
        ...
        # --- Определяем основной тип движения (ось) ---
        direction_m = target_pos_m - current_pos_m
        if np.linalg.norm(direction_m) < 1e-6:
            # Уже на месте — обработка прибытия
            self.drone_pos = self.target_pos.copy()
        ...
        # Определяем режим движения: горизонталь или вертикаль
        if abs(direction_m[0]) > abs(direction_m[1]):
        ...
        # Сохраним прежнюю позицию в метрах для расчета энергии
        prev_pos_m = self.motion_controller.pos.copy()

        # --- Шаг движения (физика происходит в метрах!) ---
        self.motion_controller.update(sim_delta_time)
        new_pos_m = self.motion_controller.pos.copy()

        # Переводим позицию обратно в клетки для визуализации и логики миссии
        self.drone_pos = new_pos_m / self.cell_size

        # --- Логика прибытия ---
        distance_left_m = np.linalg.norm(target_pos_m - new_pos_m)
        arrived = distance_left_m < 1e-2  # Можно увеличить порог до 0.5-1 м для плавности

        # --- Добавляем точку в путь (если новая) ---
        if not self.drone_path or not np.allclose(self.drone_path[-1], self.drone_pos, atol=1e-5):
            self.drone_path.append(self.drone_pos.copy())

        # --- Расход энергии за этот шаг (рассчитываем по реальному перемещению в клетках) ---
        ...
        # Если дрон прибыл, фиксируем на целевой точке, сбрасываем скорость
        if arrived:
            self.drone_pos = self.target_pos.copy()
            self.motion_controller.set_position(target_pos_m)
            self.motion_controller.set_velocity([0, 0])
    ...
    def start_mission(self, event=None) -> None:
        """Запуск миссии туда-обратно с промежуточными подзарядками и валидацией маршрута."""
        self.stop_after_forced_landing = False
        try:
            ...
            energy_to_B = self.calculate_energy_consumption(self.start_pos, self.end_pos, self.drone_height,
                                                            self.drone_height)
            remaining_after_B = self.BATTERY_CAPACITY_WATT_HOURS - energy_to_B
            energy_B_to_A = self.calculate_energy_consumption(self.end_pos, self.start_pos, self.drone_height,
                                                              self.drone_height)
            enough_for_return = remaining_after_B >= energy_B_to_A
            enough_for_station = False
            for i, station in enumerate(self.stations):
                e_station = self.calculate_energy_consumption(self.end_pos, station, self.drone_height,
                                                              self.station_heights[i])
                if remaining_after_B >= e_station:
                    enough_for_station = True
                    break
            if not enough_for_return and not enough_for_station:
                self.update_log(
                    "❌ ВНИМАНИЕ: Миссия опасна! После достижения конечной точки дрон не сможет вернуться и не сможет долететь ни до одной станции для подзарядки. Миссия не будет запущена.",
                    level="error"
                )
                self.mission_active = False
                return
    ...
    def plan_full_mission_with_charging(self, start, end):
        ...
        Добавлена проверка: если маршрут содержит (0,0) (не как старт) — сразу return None.
        """
        import numpy as np
            ...
            while queue:
                ...
                for next_idx in range(num_points):
                    ...
                    for i, st in enumerate(self.stations):
                        a = np.array(all_points[next_idx], dtype=float)
                        b = np.array(st, dtype=float)
                        if np.allclose(a, b, atol=1e-2):
                    ...
                    if energy <= max_energy:
                        ...
                        state = (next_idx, tuple(path), after_landing_height)
        ...
        if path_there and path_back:
            ...
            for pt in route[1:]:
                if not np.allclose(pt, filtered_route[-1], atol=1e-8):
                    filtered_route.append(pt)
            # Проверка: есть ли (0,0) (не как старт)
            for idx, pt in enumerate(filtered_route):
                if np.allclose(pt, [0, 0], atol=1e-2) and (idx != 0 or not np.allclose(pt, start, atol=1e-2)):
                    self.update_log("Ошибка: маршрут содержит невалидную точку (0,0). Миссия не будет запущена.",
                                    level="error")
                    return None
