...
class Simulation:
    ...
    def _log_capacity_change(self, value, note=""):
        caller = inspect.stack()[1]
        stack = ''.join(traceback.format_stack(limit=6))
        msg = (
            f"\n[DEBUG] Установка оставшегося заряда на {value:.4f} Вт·ч "
            f"(метод: {caller.function}, строка: {caller.lineno})"
            f"{' | ' + note if note else ''}\n"
            f"Стек вызова:\n{stack}\n"
        )
        if hasattr(self, "log_text"):
            ...
            print(msg)
        self._remaining_capacity_watt_hours = value

    @property
    def remaining_capacity_watt_hours(self):
        return getattr(self, '_remaining_capacity_watt_hours', self.BATTERY_CAPACITY_WATT_HOURS)

    @remaining_capacity_watt_hours.setter
    def remaining_capacity_watt_hours(self, value):
        self._log_capacity_change(value)
    ...
    def _log_capacity_change(self, value, note=""):
        ...
        stack = ''.join(traceback.format_stack(limit=6))
        msg = (
            f"\n[DEBUG] Установка оставшегося заряда на {value:.2f} Вт·ч "
            f"(метод: {caller.function}, строка: {caller.lineno})"
            f"{' | ' + note if note else ''}\n"
            f"Стек вызова:\n{stack}\n"
        )
        # Логируй только если log_text уже есть (иначе просто молча пропускай)
        if hasattr(self, "log_text"):
            self.update_log(msg)
        else:
            print(msg)
    ...
    def calculate_energy_consumption(self, start_pos: np.ndarray, end_pos: np.ndarray,
        ...
        result_watt_hours = total_joules / 3600

        self.update_log(
            f"[LOG-calc_energy] start={start_pos}, end={end_pos}, dx={dx:.2f}, dy={dy:.2f}, dist={distance:.2f}, "
            f"h0={start_height:.2f}, h1={end_height:.2f}, dh={height_diff:.2f}, horiz={horizontal:.2f}, "
            f"vert={vertical:.2f}, systems={systems:.2f}, total={total_joules:.2f}, Вт·ч={result_watt_hours:.4f}"
        )
        return result_watt_hours
    ...
    def consume_energy(self, distance: float = 0, height_change: float = 0, time_elapsed: float = 1):
        ...
        before = self.remaining_capacity_watt_hours
        self.update_log(
            f"[LOG-consume_energy] До списания: {before:.4f} Вт·ч, списываем: {energy_watt_hours:.4f} Вт·ч"
        )
        self.remaining_capacity_watt_hours = max(0.0, self.remaining_capacity_watt_hours - energy_watt_hours)
        self.update_log(
            f"[LOG-consume_energy] После списания: {self.remaining_capacity_watt_hours:.4f} Вт·ч"
        )
        if self.remaining_capacity_watt_hours <= 0:
            self.update_log("[LOG-consume_energy] Критический разряд батареи! Дрон остановился.")
            self.mission_active = False
            if hasattr(self, 'animation') and self.animation:
                self.animation.event_source.stop()
        self.update_ui()
    ...
    def perform_landing(self, frame: int) -> list:
        ...
        if self.is_charging:
            return [self.drone_icon]  # ничего не делаем, только визуализация
    ...
    def handle_arrival(self) -> None:
        self.update_log(
            f"[LOG-handle_arrival] Прибытие: drone_pos={self.drone_pos}, target_pos={self.target_pos}, remaining_capacity={self.remaining_capacity_watt_hours:.2f} Вт·ч, height={self.drone_height:.2f}"
        )
        if self.is_charging:
            return
        ...
        if not getattr(self, "returning_home", False) and self.end_pos is not None and np.linalg.norm(
                self.drone_pos - self.end_pos) < 0.1:
            ...
            need_return = self.calculate_energy_consumption(
                self.drone_pos, self.start_pos, self.drone_height, self._start_height_of_leg
            )
            can_return = self.remaining_capacity_watt_hours >= need_return
            self.update_log(
                f"[LOG-ВОЗВРАТ-АВТО] Требуется на возврат: {need_return:.2f} Вт·ч | "
                f"Остаток сейчас: {self.remaining_capacity_watt_hours:.2f} Вт·ч | "
                f"Достаточно? {'ДА' if can_return else 'НЕТ'}"
            )

            can_reach_start = can_return
        ...
        if getattr(self, "returning_home", False) and self.start_pos is not None and np.linalg.norm(
                self.drone_pos - self.start_pos) < 0.1:
    ...
    def move_drone(self, *args):
        # Логирование этапа
        self.update_log(
            f"[LOG-move_drone] pos={self.drone_pos}, height={self.drone_height}, "
            f"target={self.target_pos}, start_leg={getattr(self, '_start_pos_of_leg', '???')}, "
            f"start_energy={getattr(self, '_energy_start_of_leg', '???')}, "
            f"leg_in_progress={getattr(self, '_leg_in_progress', False)}"
        )

        if self.is_charging or not self.mission_active or self.target_pos is None:
            return [self.drone_icon, self.route_line]

        # Сохраняем стартовые параметры этапа ОДИН раз
        if not getattr(self, "_leg_in_progress", False):
            ...
            self.update_log(
                f"[LOG-move_drone] Новый этап: start_leg={self._start_pos_of_leg}, "
                f"target={self.target_pos}, height={self._start_height_of_leg}, "
                f"start_energy={self._energy_start_of_leg}"
            )
        ...
        self.update_log(
            f"[LOG-move_drone] distance_to_target={distance_to_target:.4f}, distance_step={distance_step:.4f}, "
            f"real_delta_time={real_delta_time:.4f}, sim_delta_time={sim_delta_time:.4f}"
        )

        if distance_step >= distance_to_target or distance_to_target < 1e-3:
            self.drone_pos = self.target_pos.copy()

            # --- Определяем целевую высоту для energy_used ---
            end_height = self._start_height_of_leg
            for i, station in enumerate(self.stations):
                if np.allclose(self.target_pos, np.array(station), atol=1e-2):
                    end_height = self.station_heights[i]
                    break
            else:
                if self.end_pos is not None and np.allclose(self.target_pos, self.end_pos, atol=1e-2):
                    end_height = 0.0
                elif self.start_pos is not None and np.allclose(self.target_pos, self.start_pos, atol=1e-2):
                    end_height = self._start_height_of_leg

            # 1. Сколько было до полёта
            start_energy = self._energy_start_of_leg

            # 2. Реальный расход на этот этап
            energy_used = self.calculate_energy_consumption(
                self._start_pos_of_leg, self.target_pos, self._start_height_of_leg, end_height
            )

            # 3. Сколько останется после этапа
            remaining_after_leg = max(0.0, start_energy - energy_used)

            # 4. Если это конечная точка — сразу считаем расход на возврат
            need_return = None
            can_return = None
            if self.end_pos is not None and np.allclose(self.target_pos, self.end_pos, atol=1e-2):
                need_return = self.calculate_energy_consumption(
                    self.target_pos, self.start_pos, end_height, self._start_height_of_leg
                )
                can_return = remaining_after_leg >= need_return

            self.update_log(
                f"[LOG-ЭТАП] СТАРТ: {start_energy:.2f} Вт·ч | ЭТАП: {energy_used:.2f} Вт·ч | "
                f"ОСТАТОК: {remaining_after_leg:.2f} Вт·ч"
            )
            if need_return is not None:
                self.update_log(
                    f"[LOG-ВОЗВРАТ] Требуется на возврат: {need_return:.2f} Вт·ч | "
                    f"Остаток после этапа: {remaining_after_leg:.2f} Вт·ч | "
                    f"Достаточно? {'ДА' if can_return else 'НЕТ'}"
                )

            self.remaining_capacity_watt_hours = remaining_after_leg
    ...
    def check_and_handle_feasibility(self):
        ...
        available_energy = self.remaining_capacity_watt_hours
        self.update_log(
            f"[LOG-check_and_handle_feasibility] Проверка возможности полёта: требуется {required_energy:.4f} Вт·ч, доступно: {available_energy:.4f} Вт·ч."
        )

        if required_energy > available_energy:
            self.update_log("[LOG-check_and_handle_feasibility] Недостаточно заряда для полёта до следующей точки — поиск станции для зарядки.")
            self.force_decision()
        else:
            self.update_log("[LOG-check_and_handle_feasibility] Энергии достаточно для перехода к следующей точке маршрута.")
    ...
    def charge_at_station(self, station_idx: int):
        self.update_log(f"[LOG-charge_at_station] Зарядка на станции {station_idx}, перед зарядкой: {self.remaining_capacity_watt_hours:.4f} Вт·ч")
    ...
    def charge_timer(self, remaining_time):
        self.update_log(f"[LOG-charge_timer] Таймер зарядки: осталось {remaining_time:.2f} сек, текущий заряд: {self.remaining_capacity_watt_hours:.4f} Вт·ч, charge={self.charge:.4f}")
    ...
    def complete_charge(self):
        self.update_log(f"[LOG-complete_charge] Завершение зарядки: было {self.remaining_capacity_watt_hours:.4f} Вт·ч")
    ...
    def resume_mission_after_charge(self):
        self.update_log(f"[LOG-resume_mission_after_charge] Возобновление миссии после зарядки: {self.remaining_capacity_watt_hours:.4f} Вт·ч")
