...
class Simulation:
    ...
    def handle_arrival(self) -> None:
        ...
        Двигает дрона строго по точкам маршрута self.route_points, не пропуская ни одной станции!
        ...
        # --- Пропуск одинаковых подряд точек маршрута (позиция и высота) ---
        while self.current_route_index + 1 < len(self.route_points):
            ...
            for i, st in enumerate(self.stations):
                if np.allclose(next_point, st, atol=1e-2):
                    ...
                    break
            # Сравниваем не только координаты, но и высоту!
    ...
    def complete_charge(self):
        ...
        После зарядки дрон должен продолжить маршрут (move_drone), НЕ handle_arrival!
        """
        self.charge = 1.0
        self.remaining_capacity_watt_hours = self.BATTERY_CAPACITY_WATT_HOURS
        ...
        def update_height():
            if not self.is_landing:
                ...
                # --- КЛЮЧЕВОЙ УЧАСТОК ---
                # Если только что поднялись с высоты станции, сдвинься по маршруту ТОЛЬКО если следующая точка действительно ДРУГАЯ!
                if hasattr(self, "route_points") and self.route_points and self.current_route_index is not None:
                    # Сравни: если текущая точка маршрута совпадает по координатам с текущей позицией (станцией),
                    # и это СТАНЦИЯ — перейти к следующей только если она отличается по координатам!
                    current_pt = np.round(self.drone_pos, 4)
                    route_pt = np.round(self.route_points[self.current_route_index], 4)
                    if np.allclose(current_pt, route_pt, atol=1e-2):
                        # Если следующая точка та же, то сдвигай индекс
                        if self.current_route_index + 1 < len(self.route_points):
                            next_pt = self.route_points[self.current_route_index + 1]
                            if not np.allclose(current_pt, next_pt, atol=1e-2):
                                self.current_route_index += 1
                                self.target_pos = self.route_points[self.current_route_index].copy()
                                self.update_log(
                                    f"После зарядки и подъёма: следующая цель по маршруту: {self.target_pos}")
                                self.mission_active = True
                            else:
                                # если следующая точка маршрута — опять эта же станция, перепрыгни через неё
                                self.current_route_index += 2
                                if self.current_route_index < len(self.route_points):
                                    self.target_pos = self.route_points[self.current_route_index].copy()
                                    self.update_log(f"Пропускаем повтор станции. Следующая цель: {self.target_pos}")
                                    self.mission_active = True
                                else:
                        ...
                        # Если текущая позиция не совпадает с точкой маршрута (очень маловероятно), просто продолжи маршрут.
                        if self.current_route_index + 1 < len(self.route_points):
                            self.current_route_index += 1
                            self.target_pos = self.route_points[self.current_route_index].copy()
    ...
    def resume_mission_after_charge(self):
        ...
        После зарядки не меняем вручную target_pos и не сбрасываем маршрут!
        """
        self.update_log("Продолжаем маршрут после зарядки.")
        self.is_landing = False

        # --- Проверяем: маршрут есть и индекс актуален ---
        if not hasattr(self, "route_points") or self.route_points is None or self.current_route_index is None:
            self.update_log("Ошибка: маршрут не задан или индекс не инициализирован при продолжении после зарядки!")
            self.complete_simulation()
            return

        # --- Если маршрут не окончен, двигаемся к следующей точке маршрута ---
        if self.current_route_index + 1 < len(self.route_points):
            self.current_route_index += 1
            self.target_pos = self.route_points[self.current_route_index].copy()
            self.update_log(f"Продолжаем маршрут после зарядки. Следующая цель: {self.target_pos}")

            # Проверяем хватит ли заряда (может быть уже новый маршрут после аварий)
            end_height = self.drone_height  # По умолчанию — высота дрона
            for i, station in enumerate(self.stations):
                if np.allclose(self.target_pos, station, atol=1e-2):
                    end_height = self.station_heights[i]
            ...
            # Если заряда хватает, продолжаем анимацию (mission_active=True, анимация уже запущена)
            self.mission_active = True
            # (НЕ запускать start_animation, просто продолжить)
            return
        else:
            # Если это была последняя точка маршрута — завершаем миссию
            self.update_log("Все точки маршрута пройдены после зарядки. Миссия завершена.")
            self.complete_simulation()
            return
