...
class Simulation:
    ...
    def update_stations_info(self):
        # Удаляем старые объекты, если они есть и поддерживают remove
        if hasattr(self, "station_patches"):
            for patch in self.station_patches:
                try:
                    # Только если это matplotlib Patch или PathCollection
                    if hasattr(patch, 'remove'):
                        patch.remove()
                except Exception as e:
                    print(f"Не удалось удалить патч: {e}")
    ...
    def perform_landing(self, frame: int) -> list:
        """
        Обновление состояния дрона при посадке — теперь посадка проходит постепенно, с учётом вертикальной скорости,
        энергозатрат и визуализации, до высоты станции.
        """
        # Если уже идёт зарядка, ничего не делаем
        if self.is_charging:
            return [self.drone_icon]

        # Если посадка не активна — ничего не делаем (например, уже завершена)
        if not self.is_landing:
            return [self.drone_icon]

        # --- Плавное снижение дрона до высоты станции ---
        current_time = time.time()
        elapsed_time_real = current_time - getattr(self, 'last_update_time', current_time)
        self.last_update_time = current_time

        # Шаг симуляционного времени с учётом множителя
        sim_delta_time = elapsed_time_real * self.simulation_speed_multiplier
        step = self.vertical_speed * sim_delta_time  # шаг по высоте (м)
        height_difference = self.target_height - self.drone_height

        # Определяем направление движения (снижение или подъём — вдруг станция выше)
        if abs(height_difference) <= abs(step):
            actual_step = height_difference  # последний шаг, чтобы не "перепрыгнуть"
            self.drone_height = self.target_height
            self.is_landing = False  # посадка завершена
        else:
            actual_step = step if height_difference > 0 else -step
            self.drone_height += actual_step

        # Энергорасход: учитываем подъём или снижение (смотри направление)
        self.consume_energy(
            distance=0,
        ...
        # Логирование (раз в 10 м или всегда при завершении)
        if abs(self.drone_height - getattr(self, 'last_logged_height', 0)) >= 10 or not self.is_landing:
            self.last_logged_height = self.drone_height
            self.update_log(
                f"Размер дрона: {self.drone_size:.2f}, Высота: {self.drone_height:.2f}, Целевая высота: {self.target_height:.2f}"
            )
        ...
        # Если посадка завершена, запускаем зарядку (если на высоте станции)
        ...
        # Если посадка ещё не завершена — запланировать следующий шаг
        if self.is_landing:
    ...
    def force_decision(self) -> bool:
        """Принудительная посадка на док-станцию, выбранную нейросетью, с остановкой миссии после зарядки."""
        try:
            ...
            if neural_confidence < confidence_threshold:
                ...
                if self.activate_reserve_mode():
                    ...
                    # Важно: тоже останавливаемся после посадки
