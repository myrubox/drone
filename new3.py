...
class Simulation:
    ...
    def __init__(self, root,
        ...
        """
        Инициализация Simulation: только параметры модели, состояния и физики.
        Визуализация и стили GUI — в отдельных методах!
        """
        # Ссылка на корневое окно
        self.root = root
        ...
        # --- Энергетика и физика ---
        self.HORIZONTAL_ENERGY = 10  # Дж/м — горизонтальный полет
        self.CLIMB_ENERGY = 96.83  # Дж/м — подъем
        self.DESCENT_ENERGY = 40  # Дж/м — спуск
        self.SYSTEMS_ENERGY_WATTS = 5  # Вт — потребление электроники
        ...
        self.BATTERY_CAPACITY = self.BATTERY_CAPACITY_WATT_HOURS * 3600  # Дж
        ...
        self.grid_width = 30
        self.grid_height = 20
        self.cell_size = 100  # 1 клетка = 100 м

        # --- Дрон и параметры полета ---
        self.drone_mass = 0.9
        ...
        # --- Размер визуализации дрона ---
        self.drone_size = 15
        self.max_drone_size = 20
        self.min_drone_size = 5

        # --- Скорость и таймеры ---
        self.real_speed = 15.0  # м/с
        self.vertical_speed = 2.5  # м/с
        self.simulation_speed = 1.0
        self.frame_interval = 100  # мс
        self.speed = self.real_speed / self.cell_size  # клеток/сек
        ...
        # --- Состояние дрона и маршрута ---
        self.drone_pos = np.array([15, 10], dtype=float)
        self.drone_path = [self.drone_pos.copy()]
        ...
        # --- GUI и визуализация будут инициализированы отдельно ---
        self.notebook = None
        self.tab_params = None
        self.tab_plot = None
        self.tab_sim = None
        # self.fig_map, self.ax, self.route_line и т.д. — только после вызова методов инициализации GUI

        # --- Нейросеть будет инициализирована после GUI ---
        # self.init_neural_network()      # Вызвать вручную после создания GUI, если нужно
    ...
    def setup_notebook_styles(self):
        """Устанавливает кастомные стили вкладок с контурами и активным выделением."""
        style = ttk.Style()
        style.theme_use('default')
        # Общий стиль вкладки с черным контуром
        ...
        # Карточка активной вкладки - черный контур
        style.map("Custom.TNotebook.Tab",
                  background=[("selected", "#f5f5f5"), ("active", "#eaeaea"), ("!active", "#fafafa")],
                  bordercolor=[("selected", "black"), ("active", "black"), ("!selected", "#bbbbbb")],
                  lightcolor=[("selected", "black"), ("active", "black")],
                  foreground=[("selected", "black"), ("active", "black"), ("!active", "black")],
                  borderwidth=[("selected", 3), ("active", 3), ("!selected", 2)],
                  )
        # Применяем стиль к notebook
        self.notebook.configure(style="Custom.TNotebook")
        # Вручную применяем стиль к каждой вкладке (можно не делать, если configure выше)
        for idx in range(self.notebook.index("end")):
            self.notebook.tab(idx, style="Custom.TNotebook.Tab")

    def init_parameters_tab(self):
        """Вкладка с параметрами и широкой шкалой скорости."""
    ...
    def setup_simulation_button_styles(self):
        """Создает стиль для черных контурных кнопок симуляции."""
        style = ttk.Style()
        style.configure("SimBlack.Outline.TButton",
                        borderwidth=2,
                        relief="solid",
                        foreground="black",
                        background="white",
        ...
        style.map("SimBlack.Outline.TButton",
                  background=[("active", "#e0e0e0"), ("pressed", "#d0d0d0")],
                  bordercolor=[("focus", "black"), ("!focus", "black")],
    ...
    def init_simulation_tab(self):
        ...
        # Секундомер и кнопки с черной рамкой
        self.timer_label = ttk.Label(control_frame, text="Время: 0.0 сек", style="Big.TLabel")
        self.timer_label.pack(pady=5)
        ttk.Button(control_frame, text="Начать полетное задание", style="SimBlack.Outline.TButton",
                   command=self.start_mission).pack(padx=8, pady=4, fill="x")
        ttk.Button(control_frame, text="Сброс симуляции", style="SimBlack.Outline.TButton",
                   command=self.reset_simulation).pack(padx=8, pady=4, fill="x")
        ttk.Button(control_frame, text="Принудительная посадка", style="SimBlack.Outline.TButton",
        ...
        ttk.Button(
            ...
            style="SimBlack.Outline.TButton"
    ...
    def perform_landing(self, frame: int) -> list:
        ...
        # --- Motion Controller: переход к вертикальному режиму ---
        self.motion_controller.set_axis('vertical')
        # Высота как движение вдоль Y (или как отдельная компонента - если ваша pos = [x, y], то y = высота)
        ...
        # --- Счетчик для mission_log при посадке ---
        ...
        if not self.is_landing and abs(self.drone_height - self.target_height) < 1e-2:
            for idx, height in enumerate(self.station_heights):
                if abs(self.drone_height - height) < 1e-2:
    ...
    def handle_arrival(self) -> None:
        ...
        - Если дрон прилетел к станции (позиция совпадает с одной из станций, на рабочей высоте),
          инициируется плавная посадка (движение только по высоте, X/Y остаются фиксированными).
        - Если дрон достиг конечной точки, начинается обратный путь.
        - Если дрон завершил обратный путь и вернулся на старт — миссия завершена.
        - Если есть промежуточные точки маршрута — переход к следующей.
        - В противном случае — миссия завершена.
        """
        ...
        # === 1. Проверяем: дрон рядом с одной из станций по X/Y (не по высоте!) ===
        STATION_RADIUS = 0.5  # Можно 1.0 если шаги крупные, но обычно достаточно 0.5 клетки
        for i, station in enumerate(self.stations):
            if np.linalg.norm(self.drone_pos - np.array(station)) < STATION_RADIUS:
                # 1.1. ФИКСИРУЕМ X/Y на станции (иначе дрон может начать "скакать" туда-сюда)
                ...
                # 1.2. Если НЕ на высоте станции — начинаем посадку
                if abs(self.drone_height - self.station_heights[i]) > 1e-2:
                    ...
                    # 1.3. Если уже на высоте станции — сразу начинаем зарядку
                    self.charge_at_station(i)
                    return

        # === 2. Если дрон достиг конечной точки (вперед) — начинаем обратный путь ===
            if not getattr(self, "returning_home", False) and np.linalg.norm(self.drone_pos - self.end_pos) < 1e-2:
                ...
                if can_reach_start:
                    ...
                    # Ищем ближайшую достижимую станцию
                    ...
                    for idx, st in enumerate(self.stations):
                        ...
                        if energy_to_station <= self.remaining_capacity_watt_hours and not self.station_statuses[idx]:
                            if energy_to_station < min_energy:
                                min_energy = energy_to_station
                                best_station = idx
                    if best_station is not None:
                        ...
                        self.update_log(
                            "На обратном пути после конечной точки: недостаточно заряда для полета ни до стартовой точки, ни до станции! Миссия завершена."
                        )
                        self.complete_simulation()
                        return

        # === 3. Если дрон вернулся на старт — миссия завершена ===
        if getattr(self, "returning_home", False) and np.linalg.norm(self.drone_pos - self.start_pos) < 1e-2:
        ...
        if hasattr(self, "route_points") and hasattr(self, "current_route_index"):
            if self.current_route_index + 1 < len(self.route_points):
                ...
                if self.current_route_index >= len(self.route_points):
                    self.update_log("Маршрут завершён или индекс маршрута вне диапазона. Дрон останавливается.")
                    self.mission_active = False
    ...
    def start_landing(self, station_idx):
        """
        Плавная анимация посадки дрона до высоты док-станции station_idx.
        После достижения высоты — запуск зарядки.
        """
        self.target_height = self.station_heights[station_idx]
        self.is_landing = True
        ...
        def update_landing():
            if not self.is_landing:
                ...
                self.update_log(
                    f"Дрон достиг высоты станции {station_idx + 1}: {self.target_height:.2f} м. Запуск зарядки."
                )
                self.charge_at_station(station_idx)
                return
            ...
            # Двигаемся вниз (или вверх, если высота станции выше — безопасно)
            ...
            self.update_drone_visuals()
            self.update_ui()
            self.root.after(100, update_landing)
    ...
    def move_drone(self, *args) -> list:
        ...
        # drone_pos и target_pos — в клетках, переводим в метры для физики
        ...
        if not hasattr(self, "_last_energy_check_target") or not np.allclose(self._last_energy_check_target,
            ...
            self._start_pos_of_leg = self.drone_pos.copy()  # в клетках
            self._energy_start_of_leg = self.remaining_capacity_watt_hours

            # MotionController: новая задача (позиция и цель в метрах)
        ...
        # Если целевая точка — станция, учитываем высоту станции
        ...
        # Логируем проверку энергии только один раз на этап
        ...
        # Если энергии не хватает — инициируем экстренную посадку
        if energy_to_target > available_energy:
        ...
        arrived = distance_left_m < 1e-2  # Порог можно увеличить для плавности
    ...
    def plan_full_mission_with_charging(self, start, end):
        """
        Перебирает ВСЕ разумные маршруты "туда-обратно" с любым числом остановок на док-станциях,
        учитывает перезарядку на каждой станции (заряд сбрасывается до максимального).
        Всегда сначала пробует ближайшую к старту станцию.
        Возвращает оптимальный маршрут (по суммарной энергии), либо None если ни один не выполним.
        Также подробно логирует каждый проверенный вариант.
        """
        import numpy as np
        from itertools import product, permutations

        # --- Подготовка данных ---
        # start, end: np.array([x, y])
        # self.stations: список np.array([x, y])
        # self.station_heights: список высот
        # self.BATTERY_CAPACITY_WATT_HOURS

        # Преобразуем координаты станций к numpy
        ...
        # Словарь для сопоставления координаты -> высота
        ...
        # --- Генерация ВСЕХ разумных маршрутов ---
        # - минимум: Старт → Конец → Старт
        # - максимум: Старт → [станции по пути туда, любые, без повторов подряд] → Конец → [станции по пути обратно, любые, без повторов подряд] → Старт
        # - не более одного посещения подряд одной станции, можно возвращаться на станцию по пути обратно
        # - всегда сначала пробуем ближайшую станцию
        ...
        # Определяем ближайшую станцию к start
        dists = [np.linalg.norm(start - s) for s in stations]
        sorted_station_indices = np.argsort(dists).tolist()

        # Перебор: по 0, 1 или 2 станций на каждом направлении (туда/обратно)
        candidate_routes = []
        N = len(stations)
        for n_tuda in range(0, N + 1):  # сколько станций по пути туда
            for n_obratno in range(0, N + 1):  # сколько по пути обратно
                # Перебор всех перестановок (без повторов) для пути туда
                for tuda_stops in permutations(list_station_indices(), n_tuda):
                    # Исключим маршруты с подряд одинаковыми станциями (редко, но для чистоты)
                    if len(tuda_stops) > 1 and any(
                            tuda_stops[i] == tuda_stops[i + 1] for i in range(len(tuda_stops) - 1)):
                        continue
                    # Перебор для обратного пути (после конца)
                    for obratno_stops in permutations(list_station_indices(), n_obratno):
                        if len(obratno_stops) > 1 and any(
                            ...
                            continue
                        # Маршрут: старт → станции(туда) → конец → станции(обратно) → старт
                        route_idx = [0] + [s + 1 for s in tuda_stops] + [N + 1] + [s + 1 for s in obratno_stops] + [0]
                        # Всегда сначала ближайшую станцию: если есть станции, первая в списке - ближайшая
        ...
        for route_idx in candidate_routes:
            ...
            for i in range(len(route_points) - 1):
                ...
                charge -= e
                total_energy += e
                # Если следующая точка — станция, после посадки заряд становится полным
                if is_station(route_idx[i + 1]):
    ...
    def complete_charge(self):
        """Завершение зарядки, обновление графика и подъем на рабочую высоту, пересчёт маршрута."""
        self.charge = 1.0
        self.remaining_capacity_watt_hours = self.BATTERY_CAPACITY_WATT_HOURS
        ...
        # Возврат на рабочую высоту и пересчёт маршрута
        ...
        def update_height():
            ...
            if abs(height_difference) <= abs(height_change):
                ...
                self.update_log(f"Дрон достиг рабочей высоты: {self.target_height:.2f} м. "
                                f"Флаг is_landing={self.is_landing}. Начинаем продолжение миссии.", level="info")
                self.route_points = []
                self.current_route_index = None
                self.target_pos = None
                self.check_and_handle_feasibility()
...
""" штрих после дрона а не перед, лог вниз, управление дроном, задание полета в координатах, параметры дрона поставишь выше
