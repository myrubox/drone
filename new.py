...
class Simulation:
    ...
    def perform_landing(self, frame: int) -> list:
        """
        Плавная посадка дрона на станцию с учетом энергозатрат, визуализации и правильного логирования.
        В основной лог пишутся все события, а в mission_log — только координаты X, Y, H (раз в N шагов и финально).
        """
        if self.is_charging:
            return [self.drone_icon]
        if not self.is_landing:
        ...
        # --- Расчет движения ---
        step = self.vertical_speed * sim_delta_time
        ...
        if abs(height_difference) <= abs(step):
            # Последний шаг посадки
            actual_step = height_difference
            self.drone_height = self.target_height
            self.is_landing = False
            # Завершение посадки — финальный лог только координаты!
        ...
        # --- Проверка завершения посадки и запуск зарядки ---
