...
class Simulation:
    ...
    def handle_arrival(self) -> None:
        ...
        - ВСЕГДА плавный полёт между всеми точками, включая станции.
        - Если следующая точка — станция: посадка, зарядка, подъём, и только затем снова движение.
        """
        self.update_log(
        ...
        # Проверяем, является ли следующая точка док-станцией
        is_station = False
        ...
        # НЕ ДЕЛАЙ посадку/зарядку напрямую!
        # ВСЕГДА летим горизонтально НАЧАЛЬНО на рабочей высоте!
    ...
    def complete_charge(self):
        """
        После зарядки — подъем на рабочую высоту, затем ВСЕГДА start_animation (горизонтальный полёт к следующей точке).
        """
        self.update_log(
        ...
        def update_height():
            if not self.is_landing:
                ...
                # ВАЖНО: теперь только start_animation (а не handle_arrival)!
                self.start_animation()
                return
